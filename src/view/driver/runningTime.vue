    ...
    handleChange(type) {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          let chartData = res.data.data.dateList || []
          chartData.sort((a, b) => {
            let l = parseInt(a.dayDate.split('-').join(''))
            let r = parseInt(b.dayDate.split('-').join(''))
            if (l < r) return -1
            if (l > r) return 1
            return 0
          })
          chartData = chartData.map(v => ({dayDate:v.dayDate.substring(5).replace('-', '/'),hour:v.hour}))
