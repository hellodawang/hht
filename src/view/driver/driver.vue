...
export default {
    ...
    this.$axios   // 云端设备监控信息
      ...
      .then(res => {
        ...
        res.data.data.cloudInfoList.forEach((v) => {
          ...
          // v.messList.forEach(u => u.gatherTime = u.gatherTime.slice(11, 16))
        })
        this.cloudStats = res.data.data
        console.log('cloudStats: ', this.cloudStats)
      })
      .catch(function(error) {
        console.log(error);
      });
  ...
  computed: {
    ...
    storage() {
      ...
      if (!data) {
        return [
          {name:'A',available:100,all:100,level:'优秀'},
          {name:'B',available:100,all:100,level:'优秀'},
          {name:'C',available:100,all:100,level:'优秀'},
        ]
      }
      return data.map((v) => {
